...
import { importStockBalanceExcel } from "../services/openingStockService";
import "../styles/GoodsImportModal.css";

const ACCEPTED_EXTENSIONS = [".xls", ".xlsx"];

// idle -> uploading -> processing -> done | error
function StockBalanceImportModal({ onClose, onSuccess }) {
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await importStockBalanceExcel(formData, (event) => {
      ...
      console.error("IMPORT STOCK BALANCE ERROR:", error.response?.data || error);
  ...
  const totalSkipped = result
    ? (result.skipped_existing_balance || 0) +
      (result.skipped_missing_warehouse || 0) +
      ...
      <div className="goods-import-modal">
        ...
        <div className="goods-import-body">
          <p className="goods-import-hint">
            ...
            <strong>Đơn vị tính</strong>, <strong>Cuối kỳ</strong>, <strong>Đơn giá</strong>,{" "}
            <strong>Mã kho</strong>. Vật tư chưa có sẽ được tạo mới; mã kho chưa có sẽ được
            cảnh báo (cần tạo kho trước). Mỗi vật tư chỉ có một dòng tồn trong một kho.
          </p>
            ...
            <div className="goods-import-result">
              <div className="result-cards">
                <div className="result-card result-created">
                  <span className="result-number">{result.created_balances ?? 0}</span>
                  <span className="result-label">Tồn kho tạo mới</span>
                </div>
              ...
              {result.missing_warehouse_codes?.length > 0 && (
                <div className="goods-import-error">
                  Mã kho chưa tồn tại: {result.missing_warehouse_codes.join(", ")} — hãy
                  tạo kho trước rồi import lại ({result.skipped_missing_warehouse} dòng bị
                  bỏ qua).
                </div>
              )}
              ...
              {totalSkipped > 0 && (
                <ul className="result-skip-detail">
                  {result.skipped_existing_balance > 0 && (
                    <li>
                      {result.skipped_existing_balance} dòng vật tư đã có tồn kho trong kho
                      tương ứng
                    </li>
                  )}
                  {result.skipped_duplicate_in_file > 0 && (
                    <li>
                      {result.skipped_duplicate_in_file} dòng trùng (vật tư + kho) ngay
                      trong file
                    </li>
                  )}
                  {result.skipped_incomplete > 0 && (
                    <li>{result.skipped_incomplete} dòng nhóm / thiếu dữ liệu bắt buộc</li>
                  )}
                  {result.skipped_invalid > 0 && (
              ...
              {result.created_units > 0 && (
                <ul className="result-skip-detail">
                  <li>{result.created_units} đơn vị tính được tạo mới</li>
                </ul>
              )}

              {result.existing_balance_codes?.length > 0 && (
                <details className="result-collapsible">
                  <summary>
                    Tồn kho đã tồn tại, bỏ qua ({result.skipped_existing_balance})
                  </summary>
                  <div className="result-codes">
                    {result.existing_balance_codes.join(", ")}
                  </div>
                </details>
              )}
...
export default StockBalanceImportModal;
